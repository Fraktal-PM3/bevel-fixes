...
- Deploys Hyperledger Fabric with FabConnect connector running in the K8s cluster
...
Configuration of the stack for non-default options is possible using these charts, broadly there are 2 places to make changes. For Fabric/FabConnect charts, the appropriate `values.yaml` files in the `values` directory allows for configuration of values such as the network configuration, channel settings, and Fabric-specific options. For FireFly related components the `values.yaml` file within the sub-directory for the chart (stored in `charts/`) contains the configuration for options.
